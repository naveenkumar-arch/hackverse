...
export const TeamsAdmin: React.FC = () => {
  ...
  return (
    <div className="space-y-8 py-6">
      ...
      {/* Admin Teams Table */}
      <div className="glass-card bg-slate-900/80 backdrop-blur-xl rounded-3xl p-8 border border-purple-500/20 shadow-2xl space-y-6">
            ...
            <tbody className="divide-y divide-slate-800 font-semibold">
              {filteredTeams.length > 0 ? (
                filteredTeams.map((team) => (
                  <tr key={team.id} className="hover:bg-purple-950/30 transition-colors">
                    <td className="px-6 py-4">
                      <p className="font-extrabold text-white text-sm">{team.name}</p>
                      <p className="text-[11px] font-mono font-bold text-cyan-400">
                        ID: {team.teamIdCode} &bull; Pass: {team.teamPassword}
                      </p>
                    </td>
                    <td className="px-6 py-4 text-xs font-bold text-purple-400">{team.eventName}</td>
                    <td className="px-6 py-4">
                      <p className="font-bold text-white flex items-center gap-1">
                        <Crown className="w-3.5 h-3.5 text-amber-400" /> {team.leaderName}
                      </p>
                      ...
                      <div className="flex flex-col gap-1 items-start">
                        <button
                          ...
                          className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-black ${
                            team.registrationStatus === 'VERIFIED' ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/30' : 'bg-purple-500/20 text-purple-300 border border-purple-500/30'
                          }`}
                        >
                        ...
                        <button
                          ...
                          className={`inline-flex items-center gap-1 text-[10px] font-bold ${
                            team.isLocked ? 'text-rose-400' : 'text-slate-400 hover:text-purple-300'
                          }`}
                        >
                          {team.isLocked ? <Lock className="w-3 h-3 text-rose-400" /> : <Unlock className="w-3 h-3" />}
      ...
      {editingTeam && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-xl flex items-center justify-center p-4">
          <div className="glass-card bg-slate-900/90 rounded-3xl p-6 sm:p-8 max-w-md w-full border border-purple-500/30 shadow-2xl space-y-6 text-slate-100">
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              <h3 className="text-xl font-black text-white">Edit Team Details</h3>
              <button onClick={() => setEditingTeam(null)} className="p-2 rounded-full hover:bg-slate-800 text-slate-400 hover:text-white">
                <X className="w-5 h-5" />
              </button>
            </div>
            <form onSubmit={handleSaveEdit} className="space-y-4">
              ...
              <div>
                <label className="text-xs font-bold text-slate-300 block mb-1">Team Code / ID</label>
                <input
                  type="text"
                  required
